...
// ── EMERGENCY PHONE (tel:) ─────────────────────────
/** Best-effort server log (Vercel /api/emergency-log); does not block calling. */
function astraLogEmergencyDialIntent(digits, label){
  try{
    if(typeof fetch!=='function'||typeof location==='undefined')return;
    if(!location.protocol||location.protocol==='file:')return;
    const url = (location.origin&&location.origin!=='null'?location.origin:'') + '/api/emergency-log';
